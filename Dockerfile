...
RUN echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections && \
    ...
    rm /public.key && \
    apt-get update && \
    apt-get install -y ubuntu-minimal ubuntu-standard neon-desktop neon-all plasma-workspace-wayland kwin-wayland kwin-wayland-backend-x11 kwin-wayland-backend-wayland kwin && \
